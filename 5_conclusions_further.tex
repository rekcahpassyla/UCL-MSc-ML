...
\todo[inline]{What are the implications for multi task learning of the above? That none of the above acted as good constraints? What is the meaning of alignment - if the domains are aligned, does it mean that the problem is well constrained and the embeddings should be good? Conversely if the domains are not aligned, does this mean the problem is not well constrained? }